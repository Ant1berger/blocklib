...
import { PanelBody, Spinner, Placeholder, PanelRow, TextControl, Button, SelectControl, BaseControl, __experimentalText as Text } from '@wordpress/components';
...
export default function Edit(props) {
    ...
    const { persistentID, blockName, otherAttributes, pictureID, pictureFile, pictureMime, pictureAlt, pictureSizesAttribute, pictureLoading, pictureFetchPriority, manualClasses, mediaQueries = [], renderedMediaQueries, anchor } = attributes;
...
`${query.css ? `${query.css}` : ''}`
    ...
    const onSelectImage = picture => {
        setAttributes({
            ...
            pictureFile: picture.filename, // Only for use in save.js.
    ...
    return (
        <Fragment>
            <InspectorControls>
                <PanelBody title={ __( 'Styles', 'bloclklib' ) } initialOpen={false}>
                                ...
                                <div>
                                    ...
                                    {pictureID && !imageData && (
                                        <div className="components-placeholder">
                                            <Spinner />
                                            <p>{__('Loading image...', 'blocklib')}</p>
                                        </div>
                                    )}
                                    { !! imageData?.source_url &&
                                        <Button variant="link" onClick={ open }>
                                            <img src={ db_data.siteUrl + imageData.source_url } alt={ pictureAlt } />
                                        </Button>
                                    }
                                </div>
                            ) }
            ...
            { isLoading && (
                <Placeholder
                    icon="format-image"
                    label={__('Loading image...', 'blocklib')}
            ...
            { !! pictureID && imageData?.source_url ? (
                <img
                    {...blockProps}
